...
class DDLUploader:
    ...
    async def __user_settings(self):
        ...
        # Enable GoFile as free service by default
        if 'gofile' not in self.__ddl_servers:
            self.__ddl_servers['gofile'] = (True, None)  # Enable GoFile without an API Key
        if 'streamtape' not in self.__ddl_servers:
            self.__ddl_servers['streamtape'] = (False, None)  # Optional StreamTape
    ...
    async def __upload_to_ddl(self, file_path):
        if await aiopath.isdir(file_path):
            LOGGER.info(f"Detected folder: {file_path}. Compressing into a ZIP file.")
        ...
        for serv, (enabled, api_key) in self.__ddl_servers.items():
            if enabled:
                ...
                if serv == 'gofile':
                    ...
                    gofile = Gofile(dluploader=self)
                    nlink = await gofile.upload(file_path)
